...
uint64_t prog(struct packet *pkt)
{
    ...
    uint64_t time_now = bpf_ktime_get_ns();

    struct host_map_value *host_saved_value;
    int lookup_result = bpf_map_lookup_elem(&hosts, pkt->eth.h_source, &host_saved_value);

    if (lookup_result == -1 || host_saved_value->port != pkt->metadata.in_port || time_now - host_saved_value->last_seen > TIMEOUT) // Controller needs notified
    {
    ...
    struct host_map_value host_new_value = {.port = pkt->metadata.in_port, .last_seen = time_now};
