...
class MainCLI(cmd.Cmd):
    ...
    def do_stop_topology_discovery(self, line):
        if self.application.discovery_thread and self.application.discovery_thread.is_alive():
            ...
            if self.application.cpu_time_thread and self.application.cpu_time_thread.is_alive():
                self.application.cpu_time_thread.join(5)
            if self.application.function_calls_thread and self.application.function_calls_thread.is_alive():
                self.application.function_calls_thread.join(5)
